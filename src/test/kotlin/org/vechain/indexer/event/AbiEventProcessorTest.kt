...
class AbiEventProcessorTest {
    ...
    @Test
    fun `publicDecodeEvent should decode valid event`() {
        val processor = TestableAbiEventProcessor(
            abiFiles = listOf("test-abis/VeBetterDAO-b3tr.json", "test-abis/VeBetterDAO-x-2-earn-rewards-pool.json"),
        ...
        val result = processor.publicDecodeEvent(
            event = event,
            tx = tx,
            block = block,
    ...
    @Test
    fun `publicDecodeEvent return null if no event matches`() {
        val processor = TestableAbiEventProcessor(
    ...
    @Test
    fun `publicDecodeLogEvents should decode valid log events`() {
        val processor = TestableAbiEventProcessor(
            abiFiles = listOf("test-abis/VeBetterDAO-b3tr.json", "test-abis/VeBetterDAO-x-2-earn-rewards-pool.json"),
        ...
        val block = BlockFixtures.BLOCK_B3TR_ACTION
        val tx = block.transactions.first()
        val logs = tx.outputs.flatMap { it.events }.mapIndexed { idx, l -> EventLog(
            address = l.address,
            topics = l.topics,
            data = l.data,
            meta = EventMeta(
    ...
    @Test
    fun `publicDecodeLogEvents should return empty list if no logs match`() {
        val processor = TestableAbiEventProcessor(
            abiFiles = listOf("test-abis/stringsAbis.json"),
            eventNames = listOf(),
            contractAddresses = emptyList(),
